...
namespace FuseCP.EnterpriseServer
{
    public class VirtualizationServerControllerProxmox: ControllerBase
    {
        ...
        public VirtualizationServerControllerProxmox(ControllerBase provider) : base(provider) { }
            ...
            string filterColumn, string filterValue, string sortColumn, int startRow, int maximumRows, bool recursive)
        {
            ...
            // get reader
            IDataReader reader = Database.GetVirtualMachinesPagedProxmox(
        ...
        {
            // get proxy
            VirtualizationServerProxmox vs = GetVirtualizationProxyByPackageId(packageId);
            ...
            string hostname, string osTemplate, string password, string summaryLetterEmail)
        {
            ...
            if (String.IsNullOrEmpty(hostname))
            {
                ...
                PackageSettings spaceSettings = PackageController.GetPackageSettings(packageId, PackageSettings.VIRTUAL_PRIVATE_SERVERS_PROXMOX);
            ...
            if (cntx.Groups.ContainsKey(ResourceGroups.Proxmox))
            {
            ...
            // CPU cores
            int cpuCores = cntx.Quotas[Quotas.PROXMOX_CPU_NUMBER].QuotaAllocatedValue;
            if (cpuCores == -1) // unlimited is not possible
                cpuCores = GetMaximumCpuCoresNumber(packageId);

            // RAM
            int ramMB = cntx.Quotas[Quotas.PROXMOX_RAM].QuotaAllocatedValue;
            if (ramMB == -1) // unlimited is not possible
                ramMB = DEFAULT_RAM_SIZE;

            // HDD
            int hddGB = cntx.Quotas[Quotas.PROXMOX_HDD].QuotaAllocatedValue;
            if (hddGB == -1) // unlimited is not possible
                hddGB = DEFAULT_HDD_SIZE;

            // snapshots
            int snapshots = cntx.Quotas[Quotas.PROXMOX_SNAPSHOTS_NUMBER].QuotaAllocatedValue;
            if (snapshots == -1) // unlimited is not possible
                snapshots = DEFAULT_SNAPSHOTS_NUMBER;

            bool dvdInstalled = !cntx.Quotas[Quotas.PROXMOX_DVD_ENABLED].QuotaExhausted;
            bool bootFromCD = !cntx.Quotas[Quotas.PROXMOX_BOOT_CD_ENABLED].QuotaExhausted;
            bool numLock = true;

            bool startShutdownAllowed = !cntx.Quotas[Quotas.PROXMOX_START_SHUTDOWN_ALLOWED].QuotaExhausted;
            bool pauseResumeAllowed = !cntx.Quotas[Quotas.PROXMOX_PAUSE_RESUME_ALLOWED].QuotaExhausted;
            bool rebootAllowed = !cntx.Quotas[Quotas.PROXMOX_REBOOT_ALLOWED].QuotaExhausted;
            bool resetAllowed = !cntx.Quotas[Quotas.PROXMOX_RESET_ALOWED].QuotaExhausted;
            bool reinstallAllowed = !cntx.Quotas[Quotas.PROXMOX_REINSTALL_ALLOWED].QuotaExhausted;

            bool externalNetworkEnabled = !cntx.Quotas[Quotas.PROXMOX_EXTERNAL_NETWORK_ENABLED].QuotaExhausted;
            int externalAddressesNumber = cntx.Quotas[Quotas.PROXMOX_EXTERNAL_IP_ADDRESSES_NUMBER].QuotaAllocatedValue;
            ...
            bool privateNetworkEnabled = !cntx.Quotas[Quotas.PROXMOX_PRIVATE_NETWORK_ENABLED].QuotaExhausted;
            int privateAddressesNumber = cntx.Quotas[Quotas.PROXMOX_PRIVATE_IP_ADDRESSES_NUMBER].QuotaAllocatedValue;
                ...
                bool privateNetworkEnabled, int privateAddressesNumber, bool randomPrivateAddresses, string[] privateAddresses, VirtualMachine otherSettings)
        {
            ...
            try
            {
                ...
                QuotaHelper.CheckListsQuota(cntx, quotaResults, Quotas.PROXMOX_SERVERS_NUMBER, VirtualizationErrorCodes.QUOTA_EXCEEDED_SERVERS_NUMBER);
                ...
                QuotaHelper.CheckNumericQuota(cntx, quotaResults, Quotas.PROXMOX_RAM, newRam, VirtualizationErrorCodes.QUOTA_EXCEEDED_RAM);
                QuotaHelper.CheckNumericQuota(cntx, quotaResults, Quotas.PROXMOX_HDD, hddGB, VirtualizationErrorCodes.QUOTA_EXCEEDED_HDD);
                ...
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_BOOT_CD_ALLOWED, bootFromCD, VirtualizationErrorCodes.QUOTA_EXCEEDED_CD_ALLOWED);

                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_START_SHUTDOWN_ALLOWED, startShutdownAllowed, VirtualizationErrorCodes.QUOTA_EXCEEDED_START_SHUTDOWN_ALLOWED);
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_PAUSE_RESUME_ALLOWED, pauseResumeAllowed, VirtualizationErrorCodes.QUOTA_EXCEEDED_PAUSE_RESUME_ALLOWED);
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_REBOOT_ALLOWED, rebootAllowed, VirtualizationErrorCodes.QUOTA_EXCEEDED_REBOOT_ALLOWED);
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_RESET_ALOWED, resetAllowed, VirtualizationErrorCodes.QUOTA_EXCEEDED_RESET_ALOWED);
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_REINSTALL_ALLOWED, reinstallAllowed, VirtualizationErrorCodes.QUOTA_EXCEEDED_REINSTALL_ALLOWED);

                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_EXTERNAL_NETWORK_ENABLED, externalNetworkEnabled, VirtualizationErrorCodes.QUOTA_EXCEEDED_EXTERNAL_NETWORK_ENABLED);
                QuotaHelper.CheckBooleanQuota(cntx, quotaResults, Quotas.PROXMOX_PRIVATE_NETWORK_ENABLED, privateNetworkEnabled, VirtualizationErrorCodes.QUOTA_EXCEEDED_PRIVATE_NETWORK_ENABLED);
                ...
                QuotaHelper.CheckNumericQuota(cntx, quotaResults, Quotas.PROXMOX_PRIVATE_IP_ADDRESSES_NUMBER, privateAddressesNumber, VirtualizationErrorCodes.QUOTA_EXCEEDED_PRIVATE_ADDRESSES_NUMBER);
                ...
                if (!String.IsNullOrEmpty(manageNic.NetworkId))
                {
                    ...
                    int manageIpsNumber = ServerController.GetUnallottedIPAddresses(
                            packageId, ResourceGroups.Proxmox, IPAddressPool.VpsManagementNetwork).Count;
                ...
                try
                {
                    ...
                    foreach (LibraryItem item in osTemplates)
                    {
                        if (String.Compare(item.LibraryID, osTemplateId, true) == 0)
                        {
                            ...
                            vm.OperatingSystemTemplatePath = osTemplate.Path;
                ...
                */
                ...
                //vm.OperatingSystemTemplatePath = GetCorrectTemplateFilePath(templatesPath, osTemplateFile);
                ...
                try
                {
                    ...
                    CreateServerAsyncWorkerProxmox worker = new CreateServerAsyncWorkerProxmox();
        ...
        {
            // start task
            TaskManager.StartTask(taskId, "PROXMOX", "CREATE", vm.Name, vm.Id, vm.PackageId);
            ...
            try
            {
                ...
                try
                {
                    if (vm.ExternalNetworkEnabled)
                    {
                        ...
                        VirtualMachineIPAddress PrimaryIP = new VirtualMachineIPAddress();
                        PrimaryIP.IPAddress = extNic.IPAddresses[0].IPAddress;
                        PrimaryIP.DefaultGateway = extNic.IPAddresses[0].DefaultGateway;
                        PrimaryIP.SubnetMask = extNic.IPAddresses[0].SubnetMask;
                        vm.PrimaryIP = PrimaryIP;
                    ...
                    if (vm.ManagementNetworkEnabled)
                    {
                        ...
                        if (packageIps.Count == 0)
                        {
                            ...
                            List<IPAddressInfo> ips = ServerController.GetUnallottedIPAddresses(
                                vm.PackageId, ResourceGroups.Proxmox, IPAddressPool.VpsManagementNetwork);
                ...
                // osTemplate.ProcessVolume
                // if (osTemplate.SysprepFiles != null && osTemplate.SysprepFiles.Length > 0)
                JobResult result = null;
                //ReturnCode code = ReturnCode.OK;
                TaskManager.Write("VPS_CREATE_OS_TEMPLATE", osTemplate.Name);


                #region Get VHD info - disabled - handled by Setup Script
                /*
                ...
                */
                ...
                #region Expand VHD - disabled - handled by Setup Script
                /*
                bool expanded = false;
                ...
                */
                ...
                #region Process VHD contents - disabled - handled by Setup Script
                /*
                // mount VHD
                ...
                */
                ...
                #region Send KVP - disabled - handled by Setup Script
                /*
            ...
            string managementNicMacAddress, int managementAddress)
        {
            ...
            try
            {
                ...
                VirtualizationServerProxmox vs = GetVirtualizationProxy(serviceId);
                ...
                item.SnapshotsNumber = cntx.Quotas[Quotas.PROXMOX_SNAPSHOTS_NUMBER].QuotaAllocatedValue;
                ...
                // set OS template
                //string templatesPath = settings["OsTemplatesPath"];
                //item.OperatingSystemTemplatePath = GetCorrectTemplateFilePath(templatesPath, osTemplateFile);
                try
                {
        ...
        private string EnsurePrivateVirtualSwitch(ServiceProviderItem item)
        {
            ...
            try
            {
                // create switch
                // load proxy
                VirtualizationServerProxmox vs = GetVirtualizationProxy(item.ServiceId);
        ...
        public VirtualMachine GetVirtualMachineGeneralDetails(int itemId)
        {
            ...
            // get proxy
            VirtualizationServerProxmox vps = GetVirtualizationProxy(machine.ServiceId);
        ...
        public ResultObject UpdateVirtualMachineHostName(int itemId, string hostname, bool updateNetBIOS)
        {
            ...
            try
            {
                ...
                // update NetBIOS name if required, geht mit proxmox so nicht
                /*
                if (updateNetBIOS)
                {
                ...
                try
                {
                    if (state == VirtualMachineRequestedState.TurnOff || state == VirtualMachineRequestedState.TurnOff)
                    {
                        ...
                        while (vm.State != VirtualMachineState.Off)
                        {
                            ...
                            if (maxwait == 120) vps.ChangeVirtualMachineState(machine.VirtualMachineId, VirtualMachineRequestedState.TurnOff);
                            if (maxwait > 180) return res;
                        }
                    }
                    else if (state == VirtualMachineRequestedState.Start)
                    {
                        ...
                        VirtualMachine vm = vps.GetVirtualMachine(machine.VirtualMachineId);
                        int maxwait = 0;
                        while (vm.State != VirtualMachineState.Running && maxwait < 30)
                        {
                            maxwait++;
                            System.Threading.Thread.Sleep(1000); // sleep 1 second
                            vm = vps.GetVirtualMachine(machine.VirtualMachineId);
                            if (maxwait > 120) return res;
                        }
                    }
                    else if (state == VirtualMachineRequestedState.Reboot)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var Joberror = vps.GetJob(result.Job.Id);
                                LogJobResult(res, Joberror);
                                TaskManager.CompleteResultTask(res);
                                return res;
                            }
                        }
        ...
        public ResultObject UpdateVirtualMachineConfiguration(int itemId, int cpuCores, int ramMB, int hddGB, int snapshots, bool dvdInstalled, bool bootFromCD, bool numLock, bool startShutdownAllowed, bool pauseResumeAllowed, bool rebootAllowed, bool resetAllowed, bool reinstallAllowed, bool externalNetworkEnabled, bool privateNetworkEnabled, VirtualMachine otherSettings)
        {
            ...
            vm.RemoteDesktopEnabled = otherSettings.RemoteDesktopEnabled;
        ...
        public LibraryItem[] GetLibraryDisks(int itemId)
        {
            ...
            // get proxy
            VirtualizationServerProxmox vs = GetVirtualizationProxy(vm.ServiceId);
            LibraryItem[] items = vs.GetDVDISOs(vm.VirtualMachineId);
        ...
        public ResultObject InsertDvdDisk(int itemId, string isoPath)
        {
            ...
            try
            {
                ...
                // insert DVD
                JobResult result = vs.InsertDVD(vm.VirtualMachineId, isoPath);
                if (result.ReturnValue != ReturnCode.OK)
                {
                    LogReturnValueResult(res, result);
                    TaskManager.CompleteResultTask(res);
        ...
        public VirtualMachineSnapshot GetSnapshot(int itemId, string snaphostId)
        {
            ...
            // get proxy
            VirtualizationServerProxmox vs = GetVirtualizationProxy(vm.ServiceId);
            return vs.GetSnapshot(vm.VirtualMachineId, snaphostId);
        }
        ...
        public ResultObject CreateSnapshot(int itemId)
        {
            ...
            try
            {
                ...
                if (!JobCompleted(vs, result.Job))
                {
                    var Joberror = vs.GetJob(result.Job.Id);
                    LogJobResult(res, Joberror);
                    TaskManager.CompleteResultTask(res);
                    return res;
                }
        ...
        public ResultObject DeleteSnapshot(int itemId, string snapshotId)
        {
            ...
            try
            {
                ...
                // delete snapshot
                JobResult result = vs.DeleteSnapshot(vm.VirtualMachineId, snapshotId);
                if (result.ReturnValue != ReturnCode.JobStarted)
                {
                    var Joberror = vs.GetJob(result.Job.Id);
                    LogJobResult(res, Joberror);
        ...
        public ResultObject DeleteSnapshotSubtree(int itemId, string snapshotId)
        {
            ...
            try
            {
                ...
                // take snapshot
                JobResult result = vs.DeleteSnapshotSubtree(vm.VirtualMachineId, snapshotId);
                if (result.ReturnValue != ReturnCode.JobStarted)
                {
                    LogReturnValueResult(res, result);
                    TaskManager.CompleteResultTask(res);
        ...
        {
            // load service
            int serviceId = PackageController.GetPackageServiceId(packageId, ResourceGroups.Proxmox);
        ...
        public ResultObject AddVirtualMachineExternalIPAddresses(int itemId, bool selectRandom, int addressesNumber, int[] addressIds, bool provisionKvp, int vlan)
        {
            if (addressIds == null)
            ...
            try
            {
                ...
                // send KVP config items - disabled for Proxmox
                if (provisionKvp)
                    SendNetworkAdapterKVP(itemId, "External");
            }
            catch (Exception ex)
        ...
        public ResultObject AddVirtualMachinePrivateIPAddresses(int itemId, bool selectRandom, int addressesNumber, string[] addresses, bool provisionKvp)
        {
            ...
            try
            {
                ...
                PackageContext cntx = PackageController.GetPackageContext(vm.PackageId);
                QuotaValueInfo quota = cntx.Quotas[Quotas.PROXMOX_PRIVATE_IP_ADDRESSES_NUMBER];
        ...
        {
            VirtualizationServerProxmox vs = new VirtualizationServerProxmox();
        ...
        private VirtualizationServerProxmox GetVirtualizationProxyByPackageId(int packageId)
        {
            // get service
            int serviceId = GetServiceId(packageId);
        ...
        private VirtualizationServerProxmox GetVirtualizationProxy(int serviceId)
        {
            return VirtualizationHelperProxmox.GetVirtualizationProxy(serviceId);
        }
        ...
        private bool JobCompleted(VirtualizationServerProxmox vs, ConcreteJob job)
        {
