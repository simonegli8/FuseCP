...
public class Proxmoxvps : HostingServiceProviderBase, IVirtualizationServerProxmox, IDisposable
{
    ...
    ApiClient api = null;
    public AsyncLock ApiLock = new AsyncLock();
    public ApiClient Api
    {
        get
        {
            using (var apilock = ApiLock.Lock())
            {
    ...
    public async Task<ApiClient> ApiAsync()
    {
        using (var apilock = await ApiLock.LockAsync())
        {
            if (api != null) return api;
    ...
    {
        var vm = (await ApiAsync()).NodeId(vmId);
    ...
    async Task<TunnelSocket> GetPveWebSocketAsync(ConnectedVncConnection connection)
    {
        ...
        if (api.LastResult.ResponseInError) throw new ArgumentException($"PVE Api Vncwebsocket returned error: {(await ApiAsync()).LastResult.GetError()}");
        ...
        if (connectDelay > TimeSpan.FromSeconds(10))
        {
            Debug.WriteLine("Warning: VNC Websocket access was slower than 10 seconds timeout value");
            Console.WriteLine("Warning: VNC Websocket access was slower than 10 seconds timeout value");
        }

        Debug.WriteLine($"VNC Websocket access delay: {connectDelay}");
